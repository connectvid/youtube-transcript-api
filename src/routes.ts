...
/** Build a complete output object for failed/unavailable videos (prevents schema validation errors) */
function buildErrorOutput(url: string, videoId: string, title: string | null, errorMessage: string): OutputItem {
    return {
        url,
        videoId,
        title,
        ...
        duration: null,
        thumbnailUrl: null,
        language: actorInput.language ?? 'en',
        languageName: '',
    ...
    } as OutputItem & { '#isFailed': boolean; '#errorMessage': string };
}
...
router.addHandler(LABELS.VIDEO, async ({ request, $, log, session }) => {
    ...
    // Fetch player response AND transcript in one call (same proxy session = same IP)
    const { playerResponse, transcriptXml } = await fetchTranscriptData(videoId, lang, includeAuto);

    if (!playerResponse) {
        // Fallback: try extracting from the web page HTML
        const htmlPlayerResponse = extractPlayerResponseFromHtml($ as any);
        if (!htmlPlayerResponse) {
            log.warning(`No player response for ${videoId}`);
            incrementItems();
            await Actor.pushData(buildErrorOutput(request.url, videoId, null, 'Video unavailable — no player response'));
            return;
        }
        // If we got HTML player response but no transcript, still report what we have
        const status = htmlPlayerResponse.playabilityStatus?.status;
        if (status === 'LOGIN_REQUIRED' || status === 'ERROR' || status === 'UNPLAYABLE') {
            const reason = htmlPlayerResponse.playabilityStatus?.reason ?? status;
            log.warning(`Video ${videoId} not accessible: ${reason}`);
            incrementItems();
            await Actor.pushData(buildErrorOutput(request.url, videoId, htmlPlayerResponse.videoDetails?.title ?? null, reason));
            return;
        }
    }
    ...
    if (status === 'LOGIN_REQUIRED' || status === 'ERROR' || status === 'UNPLAYABLE') {
        ...
        log.warning(`Video ${videoId} not accessible: ${reason}`);
        incrementItems();
        await Actor.pushData(buildErrorOutput(request.url, videoId, pr.videoDetails?.title ?? null, reason));
        return;
    }

    const metadata = input.includeVideoMetadata !== false ? extractVideoMetadata(pr) : { title: pr.videoDetails?.title ?? null, channelName: null, channelUrl: null, description: null, viewCount: null, likeCount: null, publishDate: null, duration: null, thumbnailUrl: null };

    const captionTracks = extractCaptionTracks(pr);
    ...
    // Parse transcript XML (already fetched with same proxy session)
